...
- truth:
  ...
  between experimental conditions). **Note:** when the TPR is requested
  at the leaf level (`only.leaf = TRUE`), the descendant leaves of the
  given nodes will be found and the TPR will be estimated on the leaf
  level.
...
- found:
  ...
  **Note:** when the TPR is requested at the leaf level
  ...
  found out and the TPR will be estimated on the leaf level.
