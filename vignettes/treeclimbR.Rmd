---
title: "Finding the optimal resolution of hierarchical hypotheses with treeclimbR"
...
phylogenies, at different levels of resolutions. It proposes multiple 
...
mean that we lose the ability to pinpoint specific species with a strong 
signal. Taken together, this motivates the development of a data-dependent
...
We start by loading the packages that will be needed in the analyses below.
...
The differential abundance (DA) workflow is suitable in situations where we have
...
case, a tree where internal nodes represent miRNA duplexes, primary transcript, 
and clusters of miRNAs for the second, and a clustering tree defined based on 
the average similarity between baseline clusters for the third).
...
summing the counts for all its descendant leaves. 
...
Next, we perform differential abundance analysis, comparing the two conditions. 
...
the p-value, and the sign of the inferred change. For the `runDA` output, we can 
generate such a table with the `nodeResult` function (where the `PValue` column
contains the p-value, and the `logFC` column provides information about the 
sign of the inferred change):
...
Next, for each of a range of values of the $t$ parameter (see [@huang2021]), 
`treeclimbR` determines a _candidate_. A candidate consists of a set of nodes, 
representing a specific pattern of aggregation. In general, higher values of 
$t$ lead to a higher degree of aggregation.
...
Note that at this point, we have not yet selected the optimal aggregation level, 
nor are we making conclusions about which of the retained nodes show a 
...
determine the one providing the optimal aggregation level [@huang2021]. 
...
`treeclimbR` also contains functionality for performing so called 'differential 
...
ds_cand_list <- lapply(seq_along(ds_tbl_list), 
                       FUN = function(x) {
                           xx <- getCand(tree = colTree(ds_tse),
...
We can then find the optimal candidate using the `evalCand` function, and list 
the top hits.
...
We find eight differentially expressed features (feature 1-8, as expected). 
...
```{r ds-check-cand}
lapply(findDescendant(colTree(ds_tse), node = c(11, 13, 5, 9), self.include = TRUE, 
